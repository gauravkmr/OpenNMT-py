...
def parse_and_output(lang, filename, tags, outputDir):
    tree = ET.parse(filename)
    root = tree.getroot()

    for tag in tags:
        lines = root.findall('.//' + tag)
        with open(outputDir + lang + '_monolingual_' + tag + '.txt', 'w') as f:
            for line in lines:
                f.write(line.text + '\n')
...
def main():
    ...
    parse_and_output(lang, filename, tags_list, outputDir)
